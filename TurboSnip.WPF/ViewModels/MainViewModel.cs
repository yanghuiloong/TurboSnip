...
public partial class MainViewModel : ObservableObject
{
    ...
    partial void OnSelectedModelChanged(ModelOption? value)
    {
        if (value != null)
        {
            // Show loading window when switching models
            ShowModelLoadingWindow(value.Filename, value.Name);
        }
    }
    
    /// <summary>
    /// Show model loading window and perform warmup.
    /// This blocks user interaction until the model is ready.
    /// </summary>
    private void ShowModelLoadingWindow(string modelFilename, string modelName)
    {
        // First, switch the model (this unloads current model)
        _llmService.SwitchModel(modelFilename);
        
        // Show loading window on UI thread
        System.Windows.Application.Current.Dispatcher.Invoke(() =>
        {
            var warmupWindow = new GpuWarmupWindow(_llmService, modelFilename, _hotkeyService);
            
            // Set owner only if MainWindow exists and is valid
    ...
    public MainViewModel(IOcrService ocrService, ILlmService llmService, IHotkeyService hotkeyService, ISnippingService snippingService, AppConfig config)
    {
        ...
        // Set callback for when hotkey is pressed during model warmup
        _hotkeyService.OnDisabledHotkeyPressed = OnHotkeyPressedDuringWarmup;
    ...
    /// <summary>
    /// Called when user presses the hotkey while model is warming up.
    /// Brings the warmup window to focus to remind user to wait.
    /// </summary>
    private void OnHotkeyPressedDuringWarmup()
    {
        System.Windows.Application.Current.Dispatcher.Invoke(() =>
        {
            // Bring the warmup window to focus
            GpuWarmupWindow.BringToFocus();
        });
    }
    ...
    {
        // Check if model is still warming up (hotkey is disabled during warmup)
        if (!_hotkeyService.IsEnabled)
        {
            // Bring warmup window to focus
            GpuWarmupWindow.BringToFocus();
            return;
        }

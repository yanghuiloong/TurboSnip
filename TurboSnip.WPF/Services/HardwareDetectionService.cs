...
public partial class HardwareDetectionService
{
    ...
    public RecommendedConfig GetRecommendation()
    {
        ...
        // Model requirements:
        // - Qwen3-4B: Needs 8GB+ VRAM (4060Ti, 3070, etc.)
        // - Qwen2.5-3B: Needs 6GB VRAM (2060, 1660 Super, etc.)
        // - Qwen2.5-1.5B: Needs 4GB VRAM (1060, 1650, etc.)

        // Tier 1: High Performance (VRAM >= 8GB) - Use 4B model
        if (hw.VramGb >= 8)
        {
        ...
        // Tier 2: Balanced (6GB <= VRAM < 8GB) - Use 3B model (NOT 4B!)
        if (hw.VramGb >= 6)
        {
            string target = PickBestAvailable(ModelMid, ModelLow); // Prefer 3B, skip 4B
            return new RecommendedConfig(
                UseGpu: true,
                ModelName: target,
        ...
        // Tier 3: Entry Level (4GB <= VRAM < 6GB) - Use 1.5B model
        if (hw.VramGb >= 4)
        {
            string target = PickBestAvailable(ModelLow, ModelMid);
            return new RecommendedConfig(
                UseGpu: true,
                ModelName: target,
                Reason: $"Entry-level GPU detected ({hw.TopGpuName}, {hw.VramGb}GB VRAM). Using lightweight model."
            );
        }

        // Tier 4: CPU Fallback (VRAM < 4GB or no GPU)

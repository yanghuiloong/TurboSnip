...
public interface IHotkeyService
{
    ...
    /// <summary>
    /// Callback triggered when hotkey is pressed while disabled (e.g., during model warmup).
    /// Use this to show a "please wait" message to the user.
    /// </summary>
    Action? OnDisabledHotkeyPressed { get; set; }
}
...
public partial class HotkeyService : IHotkeyService, IDisposable
{
    ...
    /// <summary>
    /// Enable/disable hotkey processing. When disabled, hotkey presses are ignored.
    /// </summary>
    public bool IsEnabled { get; set; } = true;
    
    /// <summary>
    /// Callback triggered when hotkey is pressed while disabled.
    /// </summary>
    public Action? OnDisabledHotkeyPressed { get; set; }
    ...
    private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        if (msg == WM_HOTKEY && wParam.ToInt32() == HOTKEY_ID)
        {
            ...
            {
                // Hotkey pressed while disabled (e.g., during warmup)
                // Notify user via callback
                OnDisabledHotkeyPressed?.Invoke();
            }
            handled = true;
        }
        return IntPtr.Zero;

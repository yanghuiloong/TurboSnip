...
public partial class LlamaService : ILlmService, IDisposable
{
    ...
    internal static string PreprocessOcrText(string input)
    {
        ...
        // 2. Regex Repair
        // 2.1 Fix lowercase-Uppercase collision (e.g. "preferAll" -> "prefer All")
        // This helps detect word boundaries that OCR missed
        text = LowerUpperRegex().Replace(text, "$1 $2");

        // 2.2 Fix sticky punctuation (e.g. "word,next" -> "word, next")
        text = StickyPunctuationRegex().Replace(text, "$1 $2");
        
        // 2.3 Fix period/colon followed by letter (e.g. "end.Start" -> "end. Start")
        text = PeriodLetterRegex().Replace(text, "$1 $2");

        // 2.4 Fix hyphenated words across lines (e.g. "pro-\ncess" -> "process")
        // Note: \s+ matches newlines, so this merges the line if split by hyphen.
        text = HyphenatedWordRegex().Replace(text, "$1$2");

        // 2.5 Fix Bullet Spacing (ensure "•Text" becomes "• Text")
        text = BulletSpacingRegex().Replace(text, "$1 $2");
        
        // 2.6 Fix number-letter collision (e.g. "2023The" -> "2023 The")
        text = NumberLetterRegex().Replace(text, "$1 $2");
        
        // 2.7 Fix letter-number collision (e.g. "Chapter3" -> "Chapter 3")
        text = LetterNumberRegex().Replace(text, "$1 $2");
        
        // 2.8 Fix closing paren/bracket followed by letter (e.g. ")next" -> ") next")
        text = CloseBracketLetterRegex().Replace(text, "$1 $2");

        // 3. Compress horizontal whitespace only (Keep newlines!)
        // This ensures multiple spaces become single space
        text = HorizontalWhitespaceRegex().Replace(text, " ");
        
        // 4. Trim trailing whitespace from each line while preserving line structure
    ...
    [System.Text.RegularExpressions.GeneratedRegex(@"([.!?:])([A-Za-z])")]
    private static partial System.Text.RegularExpressions.Regex PeriodLetterRegex();
    ...
    private static partial System.Text.RegularExpressions.Regex NumberLetterRegex();
    
    [System.Text.RegularExpressions.GeneratedRegex(@"([A-Za-z])(\d)")]
    private static partial System.Text.RegularExpressions.Regex LetterNumberRegex();
    
    [System.Text.RegularExpressions.GeneratedRegex(@"([\)\]])([A-Za-z])")]
    private static partial System.Text.RegularExpressions.Regex CloseBracketLetterRegex();
    
    [System.Text.RegularExpressions.GeneratedRegex(@"[ \t]+\n", System.Text.RegularExpressions.RegexOptions.Multiline)]
    private static partial System.Text.RegularExpressions.Regex TrailingWhitespaceRegex();
    ...
    private async Task PerformWarmupAsync(CancellationToken token)
    {
        try
        {
            ...
            try
            {
                // Deep Warmup: Generate more tokens to fully initialize CUDA kernels
                // First inference is always slow due to kernel compilation and KV cache init
                LogDebug("Starting Deep Warmup Inference (CUDA kernel compilation)...");
                OnStatusUpdated?.Invoke(GetLocalizedStatus("Status_WarmingUp"));
                
                var warmupParams = new InferenceParams 
                { 
                    MaxTokens = 20, // Generate enough tokens to warm all CUDA paths
                    AntiPrompts = ["</s>", "<|im_end|>", "<|endoftext|>"] // Stop early if model outputs end token
                };
                
                // Use a realistic warmup prompt that exercises the translation path
                string warmupPrompt = "<|im_start|>user\nTranslate: Hello<|im_end|>\n<|im_start|>assistant\n";
                
                int tokenCount = 0;
                await foreach (var _ in _executor!.InferAsync(warmupPrompt, warmupParams, token)) 
                { 
                    tokenCount++;
                }
                
                LogDebug($"Deep Warmup Complete. Generated {tokenCount} tokens.");
                OnStatusUpdated?.Invoke(GetLocalizedStatus("Status_Ready"));
            }
            finally
            {
    ...
    /// <summary>
    /// Get simplified system prompt for lightweight models (1.5B)
    /// These models struggle with complex instructions, so we use a more direct approach.
    /// </summary>
    private static string GetLightweightSystemPrompt()
    {
        // 1.5B models need very explicit, simple instructions
        // Key issue: model adds line breaks where there shouldn't be any
        return @"Translate English to Chinese.

RULES:
1. Output ONLY Chinese translation
2. SAME line breaks as input - do NOT add new lines
3. Fix broken words (""pro- cess"" = ""process"")
4. NO notes, NO ""注："", NO explanations";
    }

    /// <summary>
    /// Get standard system prompt for standard models (3B)
    /// </summary>
    private string GetStandardSystemPrompt()
    {
        if (!string.IsNullOrWhiteSpace(_config.Llm.SystemPrompt))
            return _config.Llm.SystemPrompt;
            
        return @"You are a professional English-to-Chinese translator. Your input may contain OCR errors.

STRICT RULES:
1. Output ONLY the translated Chinese text - absolutely NO notes, explanations, or comments
2. Preserve original formatting: line breaks, bullet points, numbered lists
3. Fix OCR artifacts: reconnect broken words (e.g. ""imple- mentation"" → ""implementation"")
4. Use natural, professional Chinese expressions
5. NEVER output phrases like ""Here is the translation"", ""注："", ""译文："", or any meta-commentary";
    }

    /// <summary>
    /// Get advanced system prompt for Qwen3 models (4B+)
    /// These models support deeper reasoning and can handle more nuanced instructions.
    /// </summary>
    private static string GetAdvancedSystemPrompt()
    {
        return @"You are an expert academic translator specializing in English-to-Chinese translation for technical and scholarly content.

**INPUT CONTEXT:** The text is OCR-recognized and may contain artifacts such as:
- Hyphenated line breaks (e.g., ""algo-\nrithm"" should be ""algorithm"")
- Merged words due to poor spacing detection
- Minor character recognition errors

**YOUR TASK:**
1. Translate to fluent, professional Simplified Chinese
2. Preserve the exact structure: paragraphs, bullet points, numbered lists, line breaks
3. Intelligently repair OCR errors based on context
4. Use domain-appropriate terminology (academic, technical, scientific)
5. Ensure natural Chinese sentence flow while maintaining fidelity to the original meaning

**ABSOLUTE PROHIBITION:**
- Do NOT add any meta-commentary, notes, or explanations
- Do NOT prefix with ""Translation:"", ""Here is"", ""译文："", ""注：""
...
Begin translation immediately:";
    ...
    /// <summary>
    /// Detect model tier based on model name
    /// </summary>
    private enum ModelTier { Lightweight, Standard, Advanced }
    
    private ModelTier GetModelTier()
    {
        var name = _currentModelName.ToLowerInvariant();
        
        if (name.Contains("1.5b"))
            return ModelTier.Lightweight;
        
        if (name.Contains("qwen3") || name.Contains("4b") || name.Contains("7b") || name.Contains("8b"))
            return ModelTier.Advanced;
        
        return ModelTier.Standard; // Default for 3B and others
    }
    ...
    internal string BuildPrompt(string cleanedText)
    {
        ...
        string sys = tier switch
        {
            ModelTier.Lightweight => GetLightweightSystemPrompt(),
            ModelTier.Advanced => GetAdvancedSystemPrompt(),
            _ => GetStandardSystemPrompt()
        };

        // User part contains ONLY the text to translate - no extra instructions needed
        // System prompt already defines the translation task
